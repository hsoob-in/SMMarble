...
void goForward(int player, int step)
{
     ...
     printf("Start from %i(%s) (die number: %i)\n", smm_players[player].pos, 
...
int main(int argc, const char * argv[]) {
    ...
    while ( fscanf(fp, "%s %i", food, &foodenergy)==2 ) //read a food parameter set
    {
        //store the parameter set
        printf("%s %i\n", food, foodenergy);
        smm_food_nr = smmObj_genFood(food, foodenergy);
    }
    fclose(fp);
    ...
    while ( fscanf(fp,"%s", festival)==1) //read a festival card string
    {
        //store the parameter set
        printf("%s\n", festival);
        smm_festival_nr = smmObj_genFestival(festival);
    }
    fclose(fp);
